;(function (u) {
  u.cjTextFx = function (L) {
    function M(s, r) {
      if (v.length)
        if (s) for (; v.length; ) A(v[0]), v.shift()
        else
    ...
    function A(s) {
      ...
      for (var r = s.array; r.length; )
        u(r[0]).stop(!0).removeData('callback').removeData('isOut'), r.shift()
      r = u(s.object)
    ...
    u.cjTextFx.animate = function (s, r) {
      ...
      function U() {
        ...
        if (11 === x || 12 === x || 14 === x) {
          ...
          for (b = []; 0 < g.length; )
            (h = (Math.random() * g.length) | 0), (b[b.length] = g[h]), g.splice(h, 1)
        }
